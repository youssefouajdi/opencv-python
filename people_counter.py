...
#modification de la position
    ...
    def on_opposite_sides(self, y_coord):
        ...
        return val1 or val2
#au cas ou la personne a franchis la ligne
    def did_cross_line(self, y_coord):
        if self.on_opposite_sides(y_coord):
            ...
            self.disappear_count += 1
            return NO_CHANGE_STRING
	#position de la personne par rapport a l axe x
...
def get_video():
    ...
    ap.add_argument("-v", "--video", default="video/zenital2.avi", help="chemain vers la video")
    args = vars(ap.parse_args())

    # sinon recuperer la video de la web cam si il y a po de chemain
    if args.get("video", None) is None:
        camera = cv2.VideoCapture(0)
        time.sleep(0.25)
        return camera
    # avoir la video du fichier
    else:
    ...
    #demarer la lecture de la video
        ...
        for (x, y, w, h) in peoples:
            ...
            if cy < line_point1[1]-100:
                continue
            # afficher le rectangle et le cercle
            cv2.rectangle(img, (x, y), (x + w, y + h), (255, 0, 0), 1)
            #le centre de l image c est le cercle en rouge
            ...
            if closest_person_index is not None:
                if lowest_closest_distance < LOWEST_CLOSEST_DISTANCE_THRESHOLD:
                    ...
                    if change == ENTERED_STRING:
                        ...
                        #normalement il faut ici faire les output mais sa marche po
                        #outside_count += 1
                        pass
                else:
                #rectangle personne new
